...
packages<-c("cowplot","dismo","leaflet","mapdata","OpenStreetMap","rasterVis","rdryad","rgbif","sf","tidyverse", "ggspatial", "geodata", "terra", "tidyterra")
...
pilo.dismo <- pilo.dismo[pilo.dismo$lat != 27.57333, ] # remove occurrence point in the ocean
```
...
pilo.rgbif.df <- pilo.rgbif.df[pilo.rgbif.df$y != 27.57333, ] # remove occurrence point in the ocean
```
...
rasterVis::gplot(presence.model) + 
  ...
  scale_fill_gradientn(colours=c("brown3","yellow","darkgreen"), "Probability") +
  coord_fixed(xlim = c(-97,-76), ylim = c(25,37)) +
...
colors <- c("brown3","yellow","darkgreen")
...
```{r map forest cover, echo=TRUE, message=FALSE, warning=FALSE, fig.height=6, fig.width=7, fig.align='center'}
nlcd <- rast("NLCD2020_clip.tif")

nlcd_lowres <- aggregate(nlcd, fact = 10, fun = "modal")  
nlcd_df <- as.data.frame(nlcd_lowres, xy = TRUE)
nlcd_df_forest <- nlcd_df %>% 
  filter(landcover >= 1, landcover <= 6)

ggplot() +
  # Background: NLCD raster
  geom_raster(data = nlcd_df_forest, aes(x = x, y = y), fill = "forestgreen") +
  # State outlines
  geom_polygon(data = us, aes(x = long, y = lat, group = group),
  ...
  coord_sf(xlim = c(-97,-76), ylim = c(25,37), expand = FALSE) +
  labs(title = "2020 Forest Cover and the IBWO Historic Range",
       x = "Longitude", y = "Latitude") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5)) 
```
Because the model predicted low occurrence probability at known occurrence points, I decided to explore how forest cover overlaps with the IBWO historic range. I gathered land cover data from Google Earth Engine and overlayed the IBWO historic range. The map shows forest cover in the US in 2020. The occurrence points and forest cover do not overlap as much as I had hoped. This could be because deforestation was a major cause for the drastic drop in the IBWO population. Comparing where forest cover was historically may provide better insight into the distribution of the IBWO. 
